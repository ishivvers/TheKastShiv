...
if (((momentblue[0] / momentred[0]) GT 1.2) or $
    ...
    print, 'Red multiplied by: ', momentblue[0] / momentred[0]
...
repeat begin
    goflag = 1
    womwaveparse, waveblue, wavestring, npixblue
    waveb = float(wavestring[0])
    waver = float(wavestring[1])
    womget_element, waveblue, waveb, indexblue1
    womget_element, waveblue, waver, indexblue2
    womget_element, wavered, waveb, indexred1
    womget_element, wavered, waver, indexred2
    ...
    if (waveb LT wavered[0]) or (waver GT waveblue[npixblue-1]) or $
      (waveb GE waver) then begin
...
print, ' '
print, strcompress('Average for '+string(waveb)+':'+string(waver))
...
if (brscale EQ 'b') then begin
    print, 'scale to blue'
    brlabel = 'blue'
    print, momentblue[0]/momentred[0]
...
if (brscale EQ 'r') then begin
    print, 'scale to red'
    brlabel = 'red'
    print, momentred[0]/momentblue[0]
...
plot, waveblue[indexblue1:indexblue2], fluxblue[indexblue1:indexblue2], $
  xst = 3, yst = 3, psym = 10, xtitle =  'Wavelength', color = col.white, $
