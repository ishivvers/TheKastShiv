...
endrep until (eband GE 0) and (eband LE naps-1) and (c EQ 'y')
    ...
    hoparr[hopnum].flux = multiflux[*, ap, fband]
    hoparr[hopnum].err  = multiflux[*, ap, eband]
    hoparr[hopnum].obname = objectname
    hoparr[hopnum].head = spectrim
    hoparr[hopnum].nbin = npix

active.wave = wave
active.flux = multiflux[*, ap, fband]
active.err  = multiflux[*, ap, eband]
