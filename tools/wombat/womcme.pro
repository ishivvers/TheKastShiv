...
COMMON wom_col, col
nocom = 0
print, 'This will combine two hoppers weighted by error arrays'
repeat begin
    print, ' '
    hopchoice1 = ''
    wei1 = ''
...
factor = 1d/(hoparr[hopnum1].err^2. + hoparr[hopnum2].err^2.)
factor2 = 1d/(hoparr[hopnum1].err^(-2.) + hoparr[hopnum2].err^(-2.))
comflux = factor2 * (hoparr[hopnum1].err^(-2.)*hoparr[hopnum1].flux + $
                    hoparr[hopnum2].err^(-2.)*hoparr[hopnum2].flux)
comerr = hoparr[hopnum1].err * hoparr[hopnum2].err * sqrt(factor)
