...
weiarr = fluxes
for j = 0, npix -1 do weiarr[*,j] = wei[0:i-1]
comflux = total(weiarr*fluxes,1)/total(wei)
comerr = sqrt(total((weiarr*errs)^2.,1)
printf, ulog, systime()
printf, ulog, strcompress('File: '+hoparr[hopnum1].obname+' and')
for k = 0, i-2 do begin
  printf, ulog, strcompress('File: '+hoparr[scalehops[k]].obname+' combined')
  printf, ulog, strcompress(' with weights '+string(wei))
endfor
