...
;
      ...
      BONPLY=0.D0
      print, '*** VALUE OF I OUT OF BOUNDS IN BONPLY ***'
      RETURN, bonply
      ENDIF
 
      BONPLY=A(0)
	if (i eq 0) then return, bonply
      for j = 2, i+1 do begin
       BONPLY=X*BONPLY+A(J-1)
	endfor
      RETURN, bonply
  
      END
pro   BONREBIN_TFORM, rx, x, arctwo, darcone, in21, acc, rslope, in11, in12, arcone
;
...
;
      ...
      RL=BONPLY(ARCTWO,IN21,RX)
;print, rl
jump1:     L=BONPLY(ARCONE,IN11,X)
;	print, 'l ', l
      DX=(RL-L)*RSLOPE
      X=X+DX
;      print, 'x ', x
      ...
      N=N+1
      RSLOPE=1/BONPLY(DARCONE,IN12,X)
      GOTO, jump1
      END

pro bonashrebin, wave, flux, nwave, nflux
...
;
; [2] Floating point overflow, or argument out of range in BONREBIN_TFORM when
...
;
; [4] Divide by zero in REBIN and/or BONREBIN_TFORM if the derivative of the
...
;print, 'darcone ', darcone, in12
  RSLOPE = 1/BONPLY(DARCONE, IN12, 0.2D0)
                                ;print, 'rslope ', rslope
  X1 = 0.2
  BONREBIN_TFORM, RX2, X1, arctwo, darcone, in21, acc, rslope, $
    in11, in12, arcone
                                ;print, x1
  X1 = X1*NPIX
;print, x1
  DX = 0
  NSGN = 1
  IF((BONPLY(ARCTWO, IN21, 1.0D0)-ARC2(1))*RSLOPE LT 0.0D0) then  NSGN = -1
  NSTOP = NRBIN
...
;print, j1
  for k = 0L, nstop-1 do begin
      ...
      RX2 = RX2+RNRBIN
      X2 = (X1+DX)*RNPIX
      BONREBIN_TFORM, RX2, X2, arctwo, darcone, in21, acc, $
