...
def reference_arc(image, output, reference, 
        coordlist='/indirect/big_scr5/ishivvers/kastreductions/cenko/caldir/licklinelist.dat'):
...
def red_standard(image, arcs, flats, object=None, biassec=REDBIAS, 
    ...
    reference_arc(arcs[0], arc, image[0])

    # Apply wavelength solution to standard
    iraf.refspec(object, references=arc, confirm=no, assign=yes, sort="", group="", override=yes)
...
def blue_standard(image, arcs, flats, object=None, biassec1=BLUEBIAS1,
    ...
    # Apply wavelength solution to standard
    #iraf.refspec(object, references=arc, confirm=no, assign=yes)
    iraf.refspec(object, references=arc, confirm=no, assign=yes, sort="", group="", override=yes)
    iraf.dispcor(object, '%s.w' % object, confirm=no, listonly=no)
...
def red_science(image, flats, spath, object=None, arc='Arc-Red.fits', 
    ...
    #### MODIFIED: CR rejection procedure busted!?
    #os.system("cp %s c%s" %(image[0],image[0]) )
    array, header = cosmics.fromfits(image[0])
    c = cosmics.cosmicsimage(array, gain=gain, readnoise=rdnoise,
                             sigclip = 4.5, sigfrac = 0.5,
                             objlim = 1.0)
    c.run(maxiter = 3)
    cosmics.tofits('c%s'%image[0], c.cleanarray, header)
    cosmics.tofits('cm%s'%image[0], c.mask, header)
    
    # Extract spectrum
    if object==None:
        object=get_head(image, 'OBJECT')
    iraf.apall('c%s' % image[0], output=object, references='', interactive=yes, 
               find=yes, recenter=yes, resize=yes, edit=yes, trace=yes, 
               fittrace=yes, extract=yes, extras=yes, review=yes, 
               background='fit', weights='variance', pfit='fit1d', 
               readnoise=rdnoise, gain=gain)
    ...
    iraf.telluric('%s.s' % object, '%s.t' % object, telluric, xcorr=yes,
                  tweakrms=yes, interactive=no, sample='6850:6950,7575:7700')
...
def blue_science(image, spath, object=None, flat='Flat-Blue.fits', 
    ...
    #                gain=gain, readn=rdnoise, xorder=9, yorder=3,
    #                sigclip=4.5, sigfrac=0.5, objlim=1.0, niter=3)
    #### MODIFIED: CR rejection is busted?!
    #os.system('cp j%s cj%s' %(image[0],image[0]) )
    array, header = cosmics.fromfits('j%s'%image[0])
    c = cosmics.cosmicsimage(array, gain=gain, readnoise=rdnoise,
                             sigclip = 4.5, sigfrac = 0.5,
                             objlim = 1.0)
    c.run(maxiter = 3)
    cosmics.tofits('cj%s'%image[0], c.cleanarray, header)
    cosmics.tofits('cmj%s'%image[0], c.mask, header)
 
    # Extract spectrum
    if object==None:
        object=get_head(image, 'OBJECT')
    iraf.apall('cj%s' % image[0], output=object, references='', 
               interactive=yes, find=yes, recenter=yes, resize=yes, edit=yes, 
               trace=yes, fittrace=yes, extract=yes, extras=yes, review=yes, 
               background='fit', weights='variance', pfit='fit1d', 
               readnoise=rdnoise, gain=gain)
